...
const EventDetails = () => {
  ...
  const handleBookClick = () => {
    if (!token)
      return modal.show(
        "UnAuthorized",
        "You must be logged in to bookmark this event",
        "Close"
      );
    if (role !== "user")
      return modal.show("Sorry", "Only users can bookmark events", "Close");
    ...
    const handleUpdate = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setReviews((prev) =>
            prev.map((r) =>
              r.id === editingReviewId ? { ...r, ...updated } : r
            )
          );
          cancelEditing();
        }
                    ...
                    <div>
                      ...
                      {rev.reply && (
                        <div className="ml-6 mt-2 p-2 border-l-2 border-gray-300 text-gray-600">
                          <strong>{userInfo?.username} reply:</strong>{" "}
                          {rev.reply}
                        </div>
                      )}
                    </div>
